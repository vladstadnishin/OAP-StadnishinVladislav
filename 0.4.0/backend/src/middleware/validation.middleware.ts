...
// Перетворює помилки Zod у єдиний формат details для API.
...
export function validateRequest(config: ValidationConfig) {
  return (req: Request, _res: Response, next: NextFunction) => {
    try {
      // Валідація params, query і body відбувається до контролера.
      if (config.params) {
      ...
      if (error instanceof ZodError) {
        // Усі помилки валідації зводимо до одного формату 400.
        next(
